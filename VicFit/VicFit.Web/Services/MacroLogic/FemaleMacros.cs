...
namespace VicFit.Web.Services.MacroLogic
{
    public class FemaleMacros : IMacroStrategy
    {
        ...
        public FemaleMacros(IDataProvider dataProvider, IProfileService profileService)
        {
        ...
        public int calcMacros(string userId, int profileId)
        {
            ...
            _dataProvider.ExecuteNonQuery(
                ...
                inputParamMapper: delegate (SqlParameterCollection paramList)
                {
                    ...
                    int calories = (int)(Math.Round(Convert.ToDouble((profileStats.Activity)) * (655 + (4.35 * profileStats.CurrentWeight) + (4.7 * profileStats.Height) - (4.7 * profileStats.Age))));
                    int carbs = 0;
                    int fats = 0;
                    int proteins = 0;
