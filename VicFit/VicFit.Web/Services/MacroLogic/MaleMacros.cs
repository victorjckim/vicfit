...
namespace VicFit.Web.Services.MacroLogic
{
    public class MaleMacros : IMacroStrategy
    {
        ...
        public MaleMacros(IProfileService profileService)
        {
        ...
        public MacrosViewModel calcMacros(string userId, int profileId)
        {
            ...
            if (profileStats.GoalId == 1) {
                ...
                fats = (int)Math.Round(Convert.ToDouble(0.4 * (0.8 * profileStats.CurrentWeight)));
                carbs = (calories - ((proteins * 4) + (fats * 9))) / 4;
            } else if (profileStats.GoalId == 2) {
                calories += 500;
                proteins = (int)Math.Round(Convert.ToDouble(1.1 * (0.8 * profileStats.CurrentWeight)));
                fats = (int)Math.Round(Convert.ToDouble(0.7 * (0.8 * profileStats.CurrentWeight)));
                carbs = (calories - ((proteins * 4) + (fats * 9))) / 4;
            } else {
                calories += 100;
                proteins = (int)Math.Round(Convert.ToDouble(0.9 * (0.8 * profileStats.CurrentWeight)));
                fats = (int)Math.Round(Convert.ToDouble(0.5 * (0.8 * profileStats.CurrentWeight)));
                carbs = (calories - ((proteins * 4) + (fats * 9))) / 4;
            }
